        ...
        >
          <div className="card-dark relative w-full max-w-lg p-7" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              className="absolute right-4 top-4 rounded-full border border-white/12 p-2 text-astra-white transition hover:border-white/35 hover:text-white"
            >
            ...
            <p className="mt-4 text-sm leading-6 text-astra-white">{current.message}</p>
            {notices.length > 1 ? (
              <div className="mt-6 flex items-center justify-between text-astra-white">
                <button
                  type="button"
                  onClick={() => step(-1)}
