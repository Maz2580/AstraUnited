          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <span className={`text-xs font-black uppercase tracking-[0.14em] ${isToday ? "text-astra-gold" : "text-white/72"}`}>
                  {day.short}
                </span>
                {isToday ? (
              ...
              <div className="mt-3 flex flex-1 flex-col gap-2">
                {sessions.length === 0 ? (
                  <span className="text-xs text-white/60">Rest day</span>
                ) : (
                  sessions.map((s) => {
...
function LivePill({ status }: { status: LiveStatus | null }) {
  ...
  let label = <span className="text-white/72">Training &amp; events below</span>;
  ...
  if (status?.live) {
    ...
    label = (
      <span className="text-white">
        ...
        <span className="text-white/72">· {relativeDay(daysUntil)}</span>
      </span>
    );
  }
